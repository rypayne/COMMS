//USER GUIDE: http://www.ti.com/lit/ug/swru295e/swru295e.pdf
...
static const registerSetting_t standardRegisters[]={ //configure register settings here
...
static const registerSetting_t extendedRegisters[]={ //configure extended register settings here
...
//extended registers spi
void sendSPIEX(uint8_t byteo, uint8_t datao, uint8_t rw){
  int j;
  uint8_t excom=0b01101111;
  ...
  for(j=7;j>-1;j--){ //extended register command byte
    digitalWrite(MOSI,excom>>j&1);
  ...
  for(j=7;j>-1;j--){ //extended register address byte
    digitalWrite(MOSI,byteo>>j&1);
    bytei+=(digitalRead(MISO)<<j);
    digitalWrite(SCL,HIGH);
    digitalWrite(SCL,LOW);
  }
  ...
  for(j=7;j>-1;j--){ //data byte
    digitalWrite(MOSI,datao>>j&1);
    datai+=(digitalRead(MISO)<<j);
    digitalWrite(SCL,HIGH);
    digitalWrite(SCL,LOW);
  }
...
//==================================================
//customized spi for cc1120
void sendSPI(uint8_t byteo, uint8_t datao){
  int j;
...
//force idle
  sendSPI(SIDLE,0);
  sendSPI(SNOP,0);
  while((bytei&0b111000)==MCAL) sendSPI(SNOP,0);

//register setup
  for(uint16_t i = 0;
        i < (sizeof(standardRegisters)/sizeof(registerSetting_t)); i++) {
        sendSPI(standardRegisters[i].addr, standardRegisters[i].data);
    }

   
  for(uint16_t i = 0;
        i < (sizeof(extendedRegisters)/sizeof(registerSetting_t)); i++) {
        sendSPIEX(extendedRegisters[i].addr, extendedRegisters[i].data,0);
    }

//register write check
  Serial.print("Register write test 0b00110001\n");
  sendSPI(0b10000000+PREAMBLE_CFG0,0);
...
//FUNCTION PROTOTYPES//
///////////////////////
void sendSPI(uint8_t byteo, uint8_t datao); //sends spi command. takes address in byteo and data sent in datao. if no data needs to be sent, just input 0
void sendSPIEX(uint8_t byteo, uint8_t datao, uint8_t rw); //sends spi command for extended registers. takes address in byteo, data in datao, and read or write in rw where read is 1 and write is 0. Does not require 0x2f command
void pinInit(void); //initializes pins as input and outputs
void chipStart(void); //runs start up sequence on the CC1120. Consists of resetting and configuring registers
